...
function addRowSetValue(row, RowJson) {
    ...
    //折扣额
    var depositTalmount = RowJson == "" ? ForDight(defaultDepositRate, 2) : (data.depositTalmount == null ? ForDight(defaultDepositRate, 2) : ForDight(data.depositTalmount, 2));
    //总价
...
//    }
    ...
    str += '</td>';
    //配件名称 规格
    str += '<td  colname="itemName" title="' + itemName + '">' + itemName + '</td>';
    //规格
...
function BindingAutoComplete(row) {
    $("#txtIput" + row).autocomplete("../AjaxPages/OrderList.ashx", {
        minChars: 0,
        max: 10,
        width: 512,
        autoFill: false,
        ...
        formatItem: function (data, i, total) {
            ...
            return "<table id='tbqqq' width='512px'><tr><td align='left' width='160px'>" + data.VALUE + "</td><td align='left' width='120px'>" + data.TEXT + "</td><td align='left'>" + _itemModel + "</td><td align='center' width='60px'>" + data.ParamVaule + "</td><td align='center' width='60px'>" + data.ParamVauleA + "</td></tr></table>";
        ...
        if (TblisExistItemCode(data.VALUE) && data.VALUE != divitemcode) {
            ...
            $("#" + row).find("td[colname='itemName']").text(defaultStr);
            $("#" + row).find("td[colname='itemName']").attr("title", defaultStr);
            //单价
            ...
            $("#" + row).find("td[colname='itemName']").text(data.TEXT);
            $("#" + row).find("td[colname='itemName']").attr("title", data.TEXT);
            //单价
            ...
            if (data.ParamVauleC == "N") {
                ...
                if (data.ParamVauleD != "") {
                    ...
                    if (TblisExistItemCode(data.ParamVauleD)) {
                        ...
                        $("#" + row).find("td[colname='quantity']").text(defaultStr);
                        // 毛利
                        //$("#" + row).find("td[colname='maoRi']").text(defaultStr);
...
function ItemCodeBlur(itemCode, row) {
    ...
    var defaultStr = "";
        ...
        if ($.trim(itemCode) != "") {
            $.ajax({
                type: "POST",
                url: "/AdminUI/AjaxPages/OrderList.ashx",
                ...
                success: function (result) {
                    if (result != "false") {
                        ...
                        $("#" + row).find("td[colname='itemName']").text(data.itemName);
                        $("#" + row).find("td[colname='itemName']").attr("title", data.itemName);
                        //单价
                        ...
                        var lackHtml = '';
                        var lackTitle = '';
                        //  alert(data.lackstate);
                        //                        if (data.lackstate == "N") {
                        //                            lackHtml = "lackstatus_w";
                        //                            lackTitle = "缺件待处理";
                        //                        }
                        //                        else if (data.lackstate == "Y") {
                        //                            lackHtml = "lackstatus_g";
