...
//得到退货单号
function GetReturnBillCode() {
    //获取列表
    $.ajax({
        type: "POST",
        url: "/AdminUI/AjaxPages/AddReturn.ashx",
        data: "Action=NEWRETURNBILLCODE",
        async: false,
        beforeSend: function () {
            //  $("#tblClientList").empty()
        },
        success: function (result) {
            if (result != "") {
                ...
                //遍历数据行
                if (data.Code == "0") {
                    $("#txt_cn_s_Returncode").val(data.Data);
                }
                else {
                    $("#txt_cn_s_Returncode").val("");
    ...
    $("#tblItemRows>tbody>tr").each(function (i) {
        ...
        if (itemCode != "") {
            ...
            if (flag) {
                ...
                if (jQuery("#_fixTableMain").scrollTop() > (inputTop + jQuery("#txtIput" + row).outerHeight())) {
                    //这个表达式结果为true时,表示元素已经向上滚动,并超出了可视区域.
                    $("#_fixTableMain").animate({ scrollTop: inputTop }, 50);
                }
                else if (jQuery("#_fixTableMain").scrollTop() <= (inputTop + inputHeight - jQuery("#_fixTableMain").height())) {

                    //这个表达式结果为true时,表示元素已经向下滚动,并超出了可视区域.
                    $("#_fixTableMain").animate({ scrollTop: inputTop + inputHeight}, 50);
...
function SelectImportItemList() {
    ...
    parent.$('#openIframe')[0].src = "/AdminUI/DealerSale/ImportItemListReturn.aspx";
    parent.$('#openFrameDiv').dialog('open');
}



//保存函数保存退货单saveflag:保存后新增，保存；
function SaveReturnInofo(saveflag) {
    //操作类型otype：add edit copy
    var otype = GetQueryString("type");
    //税率
    var TaxRate = $("#dpttax_rate").val();
    //退货单号
    var Returncode = $("#txt_cn_s_Returncode").val(); //退货单号 
    //退货单号 不可为空
    if (Returncode == "") {
        parent.ShowMsg('退货单号不允许为空!');
        $("#txt_cn_s_Returncode").trigger("focus");
        return false;
    }
    else {
        if (otype != "edit") {
            ...
            $.ajax({
                type: "POST",
                url: "/AdminUI/AjaxPages/AddReturn.ashx",
                data: "Action=EXISTRETURNBILLCODE&RETURNCODE=" + Returncode + "",
                async: false,
                beforeSend: function () {
                    //  $("#tblClientList").empty()
                },
                success: function (result) {
                    if (result != "") {
                        ...
                        if (data.Code == "0") {
                            ...
                            GetReturnBillCode();
                            //新的退货单号
                            var newReturnCode = $("#txt_cn_s_Returncode").val();
                            //弹出提示
                            parent.ShowMsg("退货单号（" + Returncode + "）已被占用，新的退货单号（" + newReturnCode + "）已生成！请继续保存退货单！");
                        }
    ...
    $("#tblItemRows>tbody>tr").each(function (index) {
        ...
        ////退货单单号
        var OrderCode = $("#txt_cn_s_Returncode").val();
        //行号
        var N_ROW = row;
        //物料编码
        ...
        if ($.trim(_ITEM_CODE) != "") {
            if ($.trim(_DEPOSIT_RATE) == "") {
                itemflag = true;
                $(this).find("td[colname=depositRate]").trigger("click");
                imessage = "请输入第（" + row + "）行折扣率!";
                return false;
            }
        }
        if ($.trim(_ITEM_CODE) != "") {
            if ($.trim(_DEPOSIT_AMOUNT) == "") {
                itemflag = true;
                $(this).find("td[colname=depositTalmount]").trigger("click");
                imessage = "请输入第（" + row + "）行折扣额!";
                return false;
            }
        }
    ...
    //1、保存经销商单据主表json
    var Hflag = SetReturnHeadInofo();
    //2、保存物料行json
    var Itemflag = SetReturnItems();
    //3、保存其他费用json
    var Costflag = SetReturnOtherCost();
    //4. 保存企业单据主表
    //var Comflag = SetComReturnInfo();
    //缓存成功
...
function SetReturnHeadInofo() {
    ...
    //对象
    var ReturnHead = {
        CN_S_BILLCODE: $("#txt_cn_s_Returncode").val(), //退货单号
    ...
    //保存主表信息
    HeadJsonStr = JSON.stringify(ReturnHead);
...
function SetReturnOtherCost() {
    ...
    $("input:.othercost").each(function (index) {
        //其他费用对象
        var OtherCost = {
            CN_S_BILLCODE: $("#txt_cn_s_Returncode").val(), //订单号
...
/*复制退货单历史 弹出对话框*/
function CopyHistoryReturnItemList() {
    //获得退货单类型
    var Returntype = GetQueryString("type");
    var ReturnCode = "";
    if (Returntype == "edit" || Returntype == "copy") {
        //得到订单
        ReturnCode = GetQueryString("qcode");
    }
    //显示进度条 
    parent.$("#divLoading").show();
    parent.$('#openFrameDiv').dialog({
    ...
    parent.$('#openIframe')[0].src = "/AdminUI/DealerSale/SelectHistoryReturnItemList.aspx?ReturnCode=" + ReturnCode;
    parent.$('#openFrameDiv').dialog('open');
}
